...
async function promptTextPost(req, res) {
    try {
        const { promptInput, chatHistory } = req.body;

        const promptResult = await gemini.promptText(promptInput, JSON.stringify(chatHistory));

        return res.json({
            ...promptResult,
        });
    } catch (err) {
        console.error(`Error posting prompt data for generating: `, err);
        return res.json({
            status: 0,
        });
    }
...
async function promptTextGenerate(req, res) {
    try {
        const user = req.user;
        const { studySetName, difficulty, visibility, output } = req.body;

        const deckResult = await gemini.textInputDeck(output);
...
async function generateTextPost(req, res) {
    try {
        ...
        const user = req.user
        const { studySetName, difficulty, visibility } = req.body;
